...
function validarComunidadAutonoma() {
	...
	if(expresiones.comunidadautonoma.test(comunidadautonoma.value)) {
		campos['comunidadautonoma'] = true;
		comunidadautonoma_error.style.visibility = "hidden";
	}
	else {
		campos['comunidadautonoma'] = false;
		comunidadautonoma_error.style.visibility = "visible";
  ...
  function validarFormulario()
{
	...
	if(campos.usuario && campos.password && campos.nombre && campos.apellido1 && campos.apellido2 && campos.telefono && campos.email && campos.cp && campos.provincia && campos.comunidadautonoma && campos.dni )
	{
		register.submit();
	}
	else
